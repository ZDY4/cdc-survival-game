...
#[derive(Debug, Clone)]
pub struct BlockbenchSource {
    pub absolute_path: PathBuf,
    pub relative_path: String,
    pub kind: BlockbenchSourceKind,
}

impl BlockbenchSource {
    pub fn status_label(&self) -> String {
        match self.kind {
            BlockbenchSourceKind::ExplicitBbmodel => format!("显式关联: {}", self.relative_path),
    ...
    pub fn opened_label(&self) -> String {
        match self.kind {
...
pub fn resolve_blockbench_source(
    asset_root: &Path,
    ...
    let explicit = explicit_bbmodel_link(asset_root, &gltf_relative_path)?;
...
pub fn open_model_in_blockbench(asset_root: &Path, relative_path: &str) -> Result<String, String> {
...
pub fn open_model_directory(asset_root: &Path, relative_path: &str) -> Result<PathBuf, String> {
    let model_path = resolve_existing_asset_path(asset_root, relative_path, "gltf")?;
...
pub fn open_model_in_gltf_viewer(
    repo_root: &Path,
    asset_root: &Path,
    relative_path: &str,
) -> Result<String, String> {
    let relative_path = normalize_relative_asset_path(relative_path, "gltf")?;
    ...
    if !script.is_file() {
        return Err(format!("找不到 glTF viewer 启动脚本: {}", script.display()));
    }
    ...
    if cfg!(windows) {
        Command::new("cmd")
            ...
            .spawn()
            .map_err(|error| format!("启动 glTF viewer 失败 {}: {error}", script.display()))?;
    } else {
        Command::new(&script)
            .arg("--select-model")
            .arg(&relative_path)
    ...
    Ok(format!("已在 glTF viewer 中打开: {relative_path}"))
}

pub fn save_explicit_link(
    asset_root: &Path,
    gltf_relative_path: &str,
...
pub fn clear_explicit_link(asset_root: &Path, gltf_relative_path: &str) -> Result<(), String> {
    let gltf_relative_path = normalize_relative_asset_path(gltf_relative_path, "gltf")?;
    let mut links = load_links(asset_root)?;
...
pub fn explicit_bbmodel_link(
    asset_root: &Path,
    gltf_relative_path: &str,
) -> Result<Option<String>, String> {
    let gltf_relative_path = normalize_relative_asset_path(gltf_relative_path, "gltf")?;
    Ok(load_links(asset_root)?.get(&gltf_relative_path).cloned())
}

pub fn sibling_bbmodel_relative_path(
    asset_root: &Path,
    gltf_relative_path: &str,
) -> Option<String> {
...
pub fn normalize_relative_asset_path(path: &str, extension: &str) -> Result<String, String> {
