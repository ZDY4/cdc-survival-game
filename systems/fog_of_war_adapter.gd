...
@export var fog_height_offset: float = 0.05
...
func _try_setup() -> void:
	...
	if not player:
		...
		if _setup_attempts < _MAX_ATTEMPTS:
			...
			push_error("FogOfWarAdapter: Player not found")
		return

	var bounds := _resolve_bounds()
	if bounds.is_empty():
		push_error("FogOfWarAdapter: Bounds not resolved")
		return
	...
	_fog_system.initialize(_vision_system, bounds, resolved_map_id, fog_height_offset)
...
func _find_player() -> Node3D:
	var current_scene := get_tree().current_scene
	if current_scene:
		var nodes = current_scene.find_children("*", "PlayerController3D", true, false)
