...
func spawn_from_point(point: AISpawnPoint) -> Node3D:
	...
	if _ai_manager and _ai_manager.has_method("is_character_id_valid"):
		if not bool(_ai_manager.is_character_id_valid(point.character_id)):
			push_warning(
				"[AISpawnSystem] invalid character_id '%s' at spawn point: %s" %
				[point.character_id, spawn_id]
			)
			return null

	if _ai_manager:
		actor = _ai_manager.spawn_actor(point.character_id, spawn_pos, context)
