...
const WEAPONS = {
	...
	"shotgun": {
		...
		"can_craft": true,
		"craft_recipe": {
			"materials": [{"item": "scrap_metal", "count": 15}, {"item": "component_electronic", "count": 2}],
			"time": 120
		}
	},
	...
	"assault_rifle": {
		...
		"burst_count": 3,  # 每次攻击3"
		"required_level": 6,
		"repair_materials": [{"item": "scrap_metal", "count": 10}, {"item": "component_electronic", "count": 4}],
		"can_craft": true,
		"craft_recipe": {
...
const AMMO_TYPES = {
	...
	"ammo_pistol": {
		...
		"can_craft": true,
		"craft_recipe": {
			"materials": [{"item": "scrap_metal", "count": 1}, {"item": "component_electronic", "count": 1}],
			"output": 5
		}
	},
	"ammo_shotgun": {
	...
	"ammo_rifle": {
		...
		"can_craft": true,
		"craft_recipe": {
			"materials": [{"item": "scrap_metal", "count": 2}, {"item": "component_electronic", "count": 1}],
			"output": 5
		}
	},
	"fuel": {
		...
		"can_craft": true,
		"craft_recipe": {
			"materials": [{"item": "scrap_metal", "count": 1}, {"item": "component_electronic", "count": 2}],
			"output": 10
		}
	}
}
...
func _reset_ammo():
	...
	for ammo_type in AMMO_TYPES.keys():
...
func equip_weapon(weapon_id: String):
	...
	if player_level < weapon.required_level:
		print("[Weapon] Level too low to equip " + weapon.name)
		return false
	
	# 检查是否拥有该武器
	if weapon_id != "fist" && not _has_weapon(weapon_id):
		print("[Weapon] Don't have weapon: " + weapon.name)
		return false
...
func get_equipped_weapon():
	if WEAPONS.has(equipped_weapon):
		return WEAPONS[equipped_weapon].duplicate(true)
	return WEAPONS["fist"].duplicate(true)
		...
		match effect:
			...
			"stun":
				if randf() < weapon.get("stun_chance", 0):
					result.effects_applied.append({
						"type": "stun",
						"duration": weapon.get("stun_duration", 1)
					})
			"armor_break":
...
func repair_weapon(weapon_id: String):
	...
	var weapon_template = WEAPONS[weapon_id]
	...
	for weapon in weapon_inventory:
		if weapon.id == weapon_id:
			weapon.durability = weapon_template.max_durability
			weapon_repaired.emit(weapon_id, weapon.durability)
			print("[Weapon] Repaired: " + weapon_template.name)
			return true
...
func add_weapon(weapon_id: String):
	...
	var new_weapon = {
		"id": weapon_id,
		"durability": weapon_template.max_durability,
		"enhancements": []
	}
	
	weapon_inventory.append(new_weapon)
	print("[Weapon] Added to inventory: " + weapon_template.name)
	return true
...
func craft_weapon(weapon_id: String):
	...
	var recipe = weapon.craft_recipe
	...
	print("[Weapon] Crafted: " + weapon.name)
	return true
...
func get_weapon_weight(weapon_id: String):
	if not WEAPONS.has(weapon_id):
		return 0.0
	return WEAPONS[weapon_id].get("weight", 0.0)
