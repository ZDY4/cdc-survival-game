...
func _setup_ui():
	...
	# 标题
	var title = Label.new()
	title.text = "与 %s 交易" % (_get_character_name())
	...
	npc_info_label.text = "%s的商店" % _get_character_name()
...
func _load_inventories():
	...
	var inventory: Array = _get_trade_inventory()
	for item in inventory:
		var item_name = str(item.get("id", "未知"))
...
func _update_info():
	var mood: Dictionary = _get_social_dict().get("mood", {})
	var mood_level = _get_friendliness_label(int(mood.get("friendliness", 50)))
	var discount = 0

	if trade_component:
		var charisma = 10
		discount = (charisma - 10) * 2 + (int(mood.get("friendliness", 50)) - 50)

	npc_info_label.text = "%s的商店 [友好度:%s 折扣:%d%%]" % [_get_character_name(), mood_level, discount]
...
func _on_buy_pressed():
	...
	if trade_component:
		...
		if result.success:
			total_profit -= int(result.price)
			_load_inventories()
			trade_preview_label.text = "购买成功！花费 %d" % int(result.price)
		else:
			trade_preview_label.text = "购买失败: %s" % str(result.reason)
...
func _on_sell_pressed():
	...
	if trade_component:
		...
		if result.success:
			total_profit += int(result.price)
			_load_inventories()
			trade_preview_label.text = "出售成功！获得 %d" % int(result.price)
		else:
			trade_preview_label.text = "出售失败: %s" % str(result.reason)
...
func _get_character_name() -> String:
	return str(npc_data.get("name", "商人"))

func _get_social_dict() -> Dictionary:
