...
const LOCATION_CONNECTIONS: Dictionary = {
	"safehouse": ["street_a", "street_b"],
	"street_a": ["safehouse", "supermarket", "hospital", "factory"],
	"street_b": ["safehouse", "subway", "school"],
	"supermarket": ["street_a"],
	"hospital": ["street_a"],
	"factory": ["street_a", "forest"],
	"subway": ["street_b", "ruins"],
	"school": ["street_b"],
	"forest": ["factory"],
	"ruins": ["subway"]
}
...
# ===== 地点距离（基础移动时间，小时）=====
const LOCATION_DISTANCES: Dictionary = {
	"safehouse_street_a": 0.5,
	"safehouse_street_b": 0.5,
	"street_a_supermarket": 0.5,
	"street_a_hospital": 1.0,
	"street_a_factory": 1.5,
	"street_b_subway": 1.0,
	"street_b_school": 0.5,
	"factory_forest": 2.0,
	"subway_ruins": 1.0
...
# ===== 地点风险等级 =====
const LOCATION_RISK: Dictionary = {
	"safehouse": 0,
	"street_a": 2,
	"street_b": 3,
	"supermarket": 2,
...
func travel_to(location_id: String) -> bool:
	...
	print("[MapModule] 移动完成: %s -> %s (消耗时间: %.1f小时, 食物: %d)" % [current, location_id, travel_cost.time_hours, food_required])
	return true
...
func _calculate_travel_cost(from: String, to: String) -> Dictionary:
	...
	# 计算风险（基于两地的平均风险等级）
	var base_risk = (LOCATION_RISK.get(from, 0) + LOCATION_RISK.get(to, 0)) / 2.0
...
func _get_segment_distance(from: String, to: String) -> float:
	...
	if LOCATION_DISTANCES.has(key1):
		return LOCATION_DISTANCES[key1]
	if LOCATION_DISTANCES.has(key2):
		return LOCATION_DISTANCES[key2]
	
	# 如果在LOCATION_DISTANCES中没有定义，使用直接距离
	return _get_direct_distance(from, to)
...
func find_path(from: String, to: String) -> Array:
	...
	while queue.size() > 0:
		...
		if not LOCATION_CONNECTIONS.has(current):
			continue
		
		for neighbor in LOCATION_CONNECTIONS[current]:
