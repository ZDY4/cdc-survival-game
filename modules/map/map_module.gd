...
var _navigator: GridNavigator = GridNavigator.new()
...
func _apply_travel_costs(path_cost: Dictionary) -> void:
	...
	if randf() < float(path_cost.get("encounter_chance", 0.0)):
		_trigger_travel_encounter(float(path_cost.get("risk_level", 0.0)))

func _trigger_travel_encounter(risk_level: float) -> void:
	var encounter_system = get_node_or_null("/root/EncounterSystem")
	if encounter_system:
		print("[MapModule] 旅行途中触发遭遇，风险等级: %.1f" % risk_level)
