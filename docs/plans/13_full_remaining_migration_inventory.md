...
- 部分迁移 `Esc` 关闭链路：已覆盖 selection、active dialogue、interaction menu、trade panel、container panel、stage panels、settings 和无活动 UI 时打开 settings；待补 quantity modal、discard modal、overworld prompt、pending interaction / movement 细分优先级和 blocker 诊断。
...
- 待补地图切换后的 pending 清理、active container / dialogue / trade 关闭、相机重新定位、雾战重建。
...
- 已有拾取、容器、对话、交易、场景切换、等待、攻击的第一版；待补每种行为的完整 payload、失败反馈和 UI 刷新点。
...
- 待补远程弹药：弹匣、装填、消耗弹药类型、无弹提示、reload 命令、换弹 AP 成本。
...
- 已有物品列表和基础操作；待迁移 inventory order、排序、过滤、搜索、分类、滚动条、选中物品详情。
...
- 已有 equip / unequip 命令；待迁移装备槽 UI、空槽状态、允许槽校验、主手/副手兼容、accessory 多槽。
- 待补装备详情：属性变化、武器射程、弹药、攻速、耐久、价值、外观。
- 待补装备视觉实时更新：角色模型附件、替换 body region、武器挂点、卸下恢复。
- 待补 reload equipped weapon、弹药显示、装备特效应用、卸下失败提示。

### 8.3 容器

- 已有拿取/存放第一版；待迁移容器面板完整列表、滚动、详情、数量选择、拖拽双向转移。
- 待补容器类型：地图容器、尸体容器、掉落容器、商店容器、任务容器的 id、权限和持久化差异。
- 待补容器关闭、切换地图关闭、目标消失关闭、容器为空提示。
- 待补失败提示：容器不存在、物品不足、数量非法、背包限制、权限不足。

### 8.4 交易

- 已有买卖命令；待迁移 Rust 的交易购物车：queue buy、queue sell、adjust、remove、clear、confirm。
- 待补交易 UI：店铺库存、玩家库存、已装备物品出售、价格预览、买价/卖价倍率、总价、资金变化。
- 待补交易确认：资金不足、库存不足、不可出售、装备出售需卸下或确认、部分成交策略。
- 待补交易拖拽：shop item -> buy zone、inventory/equipment -> sell zone、queued item 调整。
- 待补交易关闭：Esc、关闭按钮、对话结束、地图切换、目标不可用。
...
- 待补技能筛选、树切换、已学/可学/锁定/属性不足/点数不足状态。
- 待补技能学习确认、失败 reason、学习后热栏建议。
- 待补主动、被动、切换技能的真实效果，不再只发 `skill_used` 占位事件。

### 9.3 Hotbar

- 待迁移多组 hotbar、多槽绑定、数字键激活、清空槽、拖拽技能到槽、绑定到第一个空槽。
...
- 待补 Journal 详情：目标节点、奖励详情、进度列表、完成/失败/可交付状态、追踪 marker。
- 待补任务反馈：toast、事件日志、地图 marker、HUD 提醒、奖励动画占位。

## 11. 制作和配方

- 已有材料/技能校验和制作命令；待补配方解锁来源：任务、技能、书籍、工作台、世界 flags。
- 待补工具要求运行时：工具在背包/装备/附近容器中是否满足，工具耐久或消耗策略。
- 待补工作台要求运行时：附近 workbench / station、地图对象 props、交互打开制作台。
- 待补制作时间：即时、排队、跨回合完成、取消制作、AP / 时间消耗。
- 待补批量制作：数量选择、材料预览、最大可制作、批量输出和 XP。
- 待补制作 UI：分类、排序、搜索、配方详情、缺失原因可点击定位、完成反馈。
...
- 背包面板待补：筛选、详情、装备槽、上下文菜单、拖拽、数量弹窗、滚动条。
...
- Crafting 面板待补：分类、详情、数量、工作台、队列、完成反馈。
- Trade 面板待补：购物车、拖拽、价格预览、确认/清空、装备出售。
- Container 面板待补：双栏、拖拽、数量、关闭、空容器提示。
...
- `ContainerUI`：补双向拖拽、数量、关闭、空容器、错误提示。
- `TradeUI`：补购物车、批量确认、资金不足、装备出售、价格预览。
