...
- Crafting 面板已有配方详情、数量预览、最大可制作、分类/排序/搜索、工作台/材料/技能缺失原因、缺失原因定位和完成反馈第一版；待补批量执行和队列。
...
- `CraftingUI`：配方详情、数量预览、最大可制作、材料/工作台/技能缺失原因、缺失原因定位和完成反馈第一版已有 smoke；待补解锁来源、工具运行时、批量执行和队列。
