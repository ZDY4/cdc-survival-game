...
- [~] 跨 domain 引用校验：item、recipe、quest、dialogue、skill、character、settlement、map、overworld、shop、appearance profile 和 appearance asset path 已有第一版覆盖，`references shop <id>` 可反查店主和交易对话，`references appearance <id>` 可反查角色外观绑定；待补 world tile、更多 legacy json 字段和 asset id 映射校验。
