...
- 攻击前目标预览第一版已迁移：`Simulation.preview_attack()` 会只读返回 actor / target、射程、距离、AP 成本、弹药可用性、命中率、暴击率、预估伤害、可攻击状态和不可攻击 reason，并复用攻击合法性 / 空间 / 可见性校验，不扣 AP、不耗弹药、不推进 RNG、不进入战斗，已由 `Combat` smoke 覆盖。待补可攻击格、目标高亮、命中对象视觉标记和 UI 文案。
