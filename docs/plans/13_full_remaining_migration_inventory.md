...
- [~] 跨 domain 引用校验：item、recipe、quest、dialogue、skill、character、settlement、map、overworld、shop、world tile、appearance profile、appearance asset path 和物品 appearance `visual_asset` -> Godot glTF 已有第一版覆盖，`references shop <id>` 可反查店主和交易对话，`references world_tile <id>` 可反查地图和 overworld tile 使用点，`references appearance <id>` 可反查角色外观绑定；待补更多 legacy json 字段和地图 asset id 映射校验。
