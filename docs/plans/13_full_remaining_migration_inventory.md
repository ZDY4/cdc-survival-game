...
- 制作 UI 已有配方详情、材料/要求/时间/XP/缺失原因、数量预览、最大可制作、分类筛选、搜索、名称/分类/可制作/数量排序、完整可滚动列表和制作成功/失败反馈第一版，并纳入 `CraftingUI` smoke；待补缺失原因可点击定位。
...
- Crafting 面板已有配方详情、数量预览、最大可制作、分类/排序/搜索、工作台/材料/技能缺失原因和完成反馈第一版；待补批量执行、队列和缺失原因定位。
