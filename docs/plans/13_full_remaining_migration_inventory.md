...
- [~] ray 命中排序细节第一版已迁移：每个 picking candidate 会记录 priority、subpriority、hit fraction、ray distance、anchor noise 和 hit index，排序按 `priority -> subpriority -> hit_fraction -> anchor_noise -> hit_index` 稳定执行；trigger/transition 先给 `scene_transition`、enter/exit 类目标分配 subpriority，候选诊断进入 `runtime_control.selection_debug.picking.candidates`，并由 `PlayerInteraction` smoke 断言 sort keys、hit fraction、distance 和 anchor noise。待补 door 近似 AABB、对象锚点噪声调参、场景切换触发器更多优先级细节和真实重叠场景截图验收。
