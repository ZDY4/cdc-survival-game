...
- 选中物品操作栏第一版已迁移：数量 SpinBox、使用、装备、丢弃按钮；任务/关键物品会禁用使用和丢弃，装备和丢弃按钮通过 `InventoryUI` smoke 走 UI 触发；背包物品可拖到装备/丢弃按钮执行对应操作，丢弃支持选中物品数量 SpinBox；待补完整右键上下文菜单：拆分、检查、加入热栏、交易、存入容器。
...
- 背包面板已有筛选、搜索、详情、滚动列表、选中物品操作栏、顺序视图拖拽重排和拖到装备/丢弃按钮第一版；待补实际装备槽集成、上下文菜单、跨面板拖拽、数量弹窗 polish。
...
- `InventoryUI`：inventory order 持久化、默认顺序排序、顺序视图拖拽重排、消耗品使用按钮、选中物品装备/丢弃按钮、拖到装备/丢弃按钮、丢弃数量 SpinBox 和任务/关键物品禁用第一版已有 smoke；待补上下文菜单、完整数量弹窗、实际装备槽/容器/交易跨面板拖拽、装备详情和更完整使用反馈。
