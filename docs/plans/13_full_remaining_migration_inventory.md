...
- 部分迁移左键/右键差异：左键主交互或移动、右键打开 interaction menu、菜单外点击关闭并阻止本次世界输入已恢复；pickup、普通地图容器、corpse container、friendly actor、hostile actor、door、缺钥匙锁门、scene transition 右键菜单的可用项 / 禁用项 metadata、tooltip、hover text、中文禁用原因和 snapshot detail map 已覆盖；待补 neutral actor、纯锁门 / 缺工具、transition 权限失败等更多动态分支，以及点击外部关闭的所有 modal 分支。
