...
- 部分迁移 `UiContextMenuState`：HUD interaction menu、库存物品右键菜单、容器物品右键菜单和交易行右键菜单已暴露 `context_menu_snapshot()` / `runtime_control.context_menu`，包含 top/menu 列表、目标、来源、数量、价格、动作、禁用态和 tooltip 摘要，并由 `UIToggle` / `InventoryUI` / `ContainerUI` / `TradeUI` smoke 覆盖；容器菜单第一版支持按行拿取/存放选中数量与全部转移此项，交易菜单第一版支持检查、直接买卖和加入购物车。待补装备槽、技能条目的上下文菜单目标、动作与关闭优先级矩阵。
