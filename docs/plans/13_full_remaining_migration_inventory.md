...
- 容器关闭已覆盖 Esc、关闭按钮、目标消失关闭和切换地图关闭；空容器提示已覆盖；待补超出距离关闭。
...
- 交易关闭已覆盖 Esc、关闭按钮、目标不可用关闭和地图切换关闭；待补对话结束关闭。
...
- Container 面板已有空容器提示；待补双栏、拖拽、数量。
...
- `ContainerUI`：补双向拖拽、数量、错误提示；关闭与空容器提示已有 smoke。
