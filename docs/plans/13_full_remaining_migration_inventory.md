...
- 尸体掉落合并和元数据第一版已迁移：尸体会合并目标背包、装备、已装填弹匣余弹和 character `combat.loot` 掉落；尸体 snapshot / container 会保留 display name、source actor id / definition id / kind、defeated by actor id、map id、grid、appearance / model asset、equipped slots 和 money；已由 `Combat` smoke 覆盖。待补金钱作为可拿取经济条目、单件耐久状态、掉落随机公式与 Rust 完整复核、尸体容器 UI 展示金钱和装备来源。
...
- 待补目标预览 UI polish：范围格世界高亮、命中 actor outline、失败 reason 文案、鼠标/键盘确认提示和目标选择音效。
...
- AI 行为事件和诊断 payload 第一版已迁移：`ai_intent_decided` 会暴露 intent、reason、target、target_grid、distance、aggro_range、attack_range、AP、path、weapon、ammo 和 reload 状态，占位空值保持稳定；已由 `AI` smoke 覆盖。待补路径失败细分、连续追踪目标、目标丢失原因和 UI 展示。
...
- 已有 Skills 面板 hotbar 可用/冷却/资源不足不可用原因文本、按钮禁用和技能 `activation.ap_cost` / `activation.resource_costs` 展示与扣除第一版；HUD hotbar 槽位已显示 key、技能短名、cooldown 文本、slot tooltip、冷却禁用态和冷却遮罩，主动技能激活后会落到 actor active effects，并纳入 `SkillsUI` / `UI` smoke。待补多组 hotbar 的资源消耗汇总展示。
...
- Journal 详情第一版已迁移：目标节点、任务描述、目标类型/需求、进度、可交付状态、奖励详情、本地追踪 marker、HUD 追踪行、地图面板追踪行、地图目标 marker、已完成任务历史和手动交付后的完成/奖励反馈已纳入 `JournalUI` / `UI` smoke；待补失败历史和更完整进度列表。
- 地图目标 marker 第一版已迁移：追踪 collect 目标会在当前地图标出匹配 pickup 或含目标物品的容器，追踪 kill 目标会标出当前地图匹配 enemy_type 的 actor；找不到目标时保留 unresolved marker 和 reason，已纳入 `JournalUI` smoke。待补 canvas 图形标记、跨地图/overworld 路线、目标优先级和完成/失败反馈。
...
- 地图面板已有当前地图、当前地点名称、入口、已解锁地点名称、对象统计、追踪任务行和追踪目标 marker 行第一版；待补 canvas、pan、zoom、overworld 路线和图形化地图目标 marker。
- Journal 面板已有任务详情、可交付状态、奖励详情、本地追踪 marker、HUD 追踪行、地图面板追踪行、地图目标 marker、已完成任务历史和手动交付完成/奖励反馈第一版；待补失败历史和更完整失败反馈。
...
- `SkillsUI`：HUD/Skills 热栏绑定、拖拽技能到 HUD 热栏槽、数字键激活、slot tooltip、cooldown 文本/禁用态、HUD 冷却遮罩、选中技能详情、技能学习确认、被动技能效果写入 actor snapshot、主动技能效果写入 actor snapshot、技能资源消耗和 `skill_used` effect/resource payload 已有 smoke；待补多组 hotbar、技能树 pan、目标预览和更完整状态 UI。
- `JournalUI`：任务详情、目标需求、奖励详情、可交付状态、本地追踪 marker、HUD 追踪行、地图面板追踪行、地图目标 marker、已完成任务历史和手动交付完成/奖励反馈第一版已有 smoke；待补对话交付条件、失败历史和更完整失败反馈。
