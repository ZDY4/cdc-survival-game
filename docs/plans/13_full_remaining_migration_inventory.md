...
- [~] UI 图标资产：`AssetPathResolver.resolve_media_asset()` 第一版已迁移，inventory item icon 与 dialogue portrait snapshot 会暴露 `icon_asset` / `portrait_asset` 诊断，能识别 legacy root `assets/...` 路径、缺失资源、fallback key 和最终 Godot `res://assets/...` 候选路径，并由 `ContentCLI` / `InventoryUI` / `DialogueUI` smoke 覆盖；待补 skill、quest、crafting、trade、container、settings、hotbar 的正式图标资源落地、manifest 和 UI 真实贴图替换。
...
- asset id -> Godot resource path 映射表第一版已迁移：新增 `AssetPathResolver` 统一把 `builtin:weapon:*`、`builtin:item:*`、`res://assets/...` 和 `godot/assets` 相对 `.gltf` 路径解析为 relative / resource / absolute path，并拒绝根目录 `assets/...`、绝对路径、逃逸路径和非 `.gltf` 正式模型引用；UI icon / dialogue portrait media resolver 第一版已迁移，会解析 `png` / `jpg` / `jpeg` / `webp` / `svg`，识别 legacy root `assets/...`、缺失资源和 domain fallback key；`WorldSnapshotBuilder`、item appearance 校验、appearance base model 校验、world tile source 校验、inventory snapshot 和 dialogue snapshot 已共用该 resolver，`ContentCLI` / `InventoryUI` / `DialogueUI` smoke 覆盖 builtin 映射、`res://assets` 归一、root assets 拒绝、missing asset 状态和 legacy media 诊断。待补 asset id manifest 导出、正式 UI media 资源落地和更多 domain-specific asset id。
