...
- [~] UI 图标资产：`AssetPathResolver.resolve_media_asset()` 第一版已迁移，inventory item icon、dialogue portrait 和 overworld location icon snapshot 会暴露 `icon_asset` / `portrait_asset` 诊断；旧 data 中 `assets/icons/*.png` / `assets/portraits/*.png` 已迁为 `res://assets/**/*.svg`，并在 `godot/assets/icons` / `godot/assets/portraits` 落地 Godot 运行时占位 SVG；`ContentCLI` / `InventoryUI` / `DialogueUI` / `UIToggle` smoke 覆盖 legacy root 诊断、fallback key、Godot resource path 和资源存在性。待补 skill、quest、crafting、trade、container、settings、hotbar 的 UI 真实贴图替换、manifest 和更细分图标表现。
...
- asset id -> Godot resource path 映射表第一版已迁移：新增 `AssetPathResolver` 统一把 `builtin:weapon:*`、`builtin:item:*`、`res://assets/...` 和 `godot/assets` 相对 `.gltf` 路径解析为 relative / resource / absolute path，并拒绝根目录 `assets/...`、绝对路径、逃逸路径和非 `.gltf` 正式模型引用；UI icon / dialogue portrait / overworld location media resolver 第一版已迁移，会解析 `png` / `jpg` / `jpeg` / `webp` / `svg`，识别 legacy root `assets/...`、缺失资源和 domain fallback key；旧 icon / portrait data 引用已迁入 `res://assets/**/*.svg`，并落地占位 SVG 资源；`WorldSnapshotBuilder`、item appearance 校验、appearance base model 校验、world tile source 校验、inventory snapshot、dialogue snapshot 和 map snapshot 已共用该 resolver，`ContentCLI` / `InventoryUI` / `DialogueUI` / `UIToggle` smoke 覆盖 builtin 映射、`res://assets` 归一、root assets 拒绝、missing asset 状态、legacy media 诊断和已迁 media 资源存在性。待补 asset id manifest 导出、真实 UI media 资源替换和更多 domain-specific asset id。
