...
- [~] `changed` / `diff-summary` 等旧 content_tools 行为：`format changed` 和 `diff-summary --path` 已有第一版；`validate changed` 已改为基于 Git status 的受支持内容路径筛选、去重、缺失记录报告和 no-supported-changes 状态，并能区分 modified / added / untracked / deleted / renamed 内容文件，deleted 会输出 `content_file_deleted`，renamed 会保留 source path 并输出 `renamed_content_file_not_loaded`；已由 `ContentCLI` smoke 与真实 `godot-content.ps1 validate changed` 覆盖。待补旧工具全部输出字段兼容、批量修复入口、更多真实仓库 deleted / renamed fixture 和批量变更摘要。
