...
- 已有拿取/存放和容器/背包双栏展示第一版；待迁移容器面板滚动、详情、数量选择、拖拽双向转移。
...
- Container 面板已有空容器提示和容器/背包双栏；待补拖拽、数量、滚动和详情。
...
- `ContainerUI`：补双向拖拽、数量、滚动/详情、背包限制/权限等高级错误；关闭、超距关闭、空容器、双栏与基础失败提示已有 smoke。
