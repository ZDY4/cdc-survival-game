...
- [~] 安全写回：格式化 dry-run 已迁移到 `content_cli format --dry-run` / `godot-content.ps1 -DryRun`，支持单记录和 `changed` 批量预览，输出 `dry_run`、单记录 `changed`、批量 `would_rewrite_files` 且不落盘；批量修复入口 `fix changed` 已有第一版，会复用格式化修复并报告 schema migration pending；schema migration 诊断已输出迁移前后 roundtrip diff 摘要，包括新增 / 移除 / 变更字段计数和字段名，CLI 会打印 `schema_roundtrip_diff` / `schema_roundtrip_added_fields`；`ContentWriteService` 已改为同目录临时文件写入后替换目标，并向 edit service 透传 `write_mode`，smoke 覆盖 dry-run 不落盘和临时文件清理；diff summary 和格式化失败不落盘已有第一版。待补真实 schema migration 写回、更多自动修复种类和失败注入 fixture。
...
- [~] 内容版本和 schema migration：新增 `ContentSchemaMigration` 第一版，`validate` 会输出 `schema_migration` 诊断并在 CLI 显示 schema status；当前无版本内容会归类为 `legacy_missing_version`，补充 `schema_version` 缺省、deprecated `schemaVersion` / `version` 检测、迁移日志、roundtrip 安全摘要和 roundtrip 字段 diff，并由 `ContentCLI` smoke 与真实 `godot-content.ps1 validate item 1006` 覆盖。待补真实批量写回 schema_version、按 domain 的旧字段迁移器、JSON 行列号和迁移日志持久化。
