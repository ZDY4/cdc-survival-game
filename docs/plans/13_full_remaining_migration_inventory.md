...
- [~] ray 命中排序细节第一版已迁移：每个 picking candidate 会记录 priority、subpriority、door AABB distance、hit fraction、ray distance、anchor noise 和 hit index，排序按 `priority -> subpriority -> door_aabb_distance -> hit_fraction -> anchor_noise -> hit_index` 稳定执行；trigger/transition 先给 `scene_transition`、enter/exit 类目标分配 subpriority；door 会根据占用 cells 计算近似水平 AABB 距离，同优先级下优先选择命中门占用范围内的候选；多格对象的 anchor noise 会优先使用 cells 中心，避免只按左上 anchor 放大噪声；候选诊断进入 `runtime_control.selection_debug.picking.candidates`，并由 `PlayerInteraction` smoke 断言 sort keys、hit fraction、distance、door AABB distance 和 anchor noise。待补场景切换触发器更多优先级细节、真实重叠场景截图验收和更细调参。
