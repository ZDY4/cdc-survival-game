...
- 部分迁移左键/右键差异：左键主交互或移动、右键打开 interaction menu、菜单外点击关闭并阻止本次世界输入已恢复；pickup 右键菜单的可用项 / 禁用项 metadata、tooltip、hover text 和 snapshot detail map 已覆盖；待补 container / actor / door / transition 等更多目标类型上下文菜单项，以及点击外部关闭的所有 modal 分支。
