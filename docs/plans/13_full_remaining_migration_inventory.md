...
- 部分迁移左键/右键差异：左键主交互或移动、右键打开 interaction menu、菜单外点击关闭并阻止本次世界输入已恢复；pickup、corpse container、hostile actor、door、scene transition 右键菜单的可用项 / 禁用项 metadata、tooltip、hover text 和 snapshot detail map 已覆盖；待补 friendly / neutral actor、普通地图容器、锁门/权限失败、transition 权限失败等更多动态分支，以及点击外部关闭的所有 modal 分支。
