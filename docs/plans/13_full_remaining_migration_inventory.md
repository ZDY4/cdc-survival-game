...
- [~] 安全写回：格式化 dry-run 已迁移到 `content_cli format --dry-run` / `godot-content.ps1 -DryRun`，支持单记录和 `changed` 批量预览，输出 `dry_run`、单记录 `changed`、批量 `would_rewrite_files` 且不落盘；diff summary 和格式化失败不落盘已有第一版。待补原子替换、批量修复入口和 schema migration 写回前后的 roundtrip diff。
