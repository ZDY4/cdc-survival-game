...
- 已有拿取/存放、容器/背包双栏、滚动列表、基础详情文本、选中详情和数量选择第一版；待迁移容器面板拖拽双向转移。
- 待补容器类型：地图容器、尸体容器、掉落容器、商店容器、任务容器的 id、权限和持久化差异。
- 容器关闭已覆盖 Esc、关闭按钮、目标消失关闭、切换地图关闭和超出距离关闭；空容器提示已覆盖。
- 基础失败提示已覆盖并纳入 `ContainerUI` smoke：容器/背包物品不足、未知容器、未知物品、未知角色、未打开容器；待补数量非法、背包限制、权限不足。

### 8.4 交易

- 已有买卖命令、店铺/玩家双栏、数量直买直卖和价格预览第一版；待迁移 Rust 的交易购物车：queue buy、queue sell、adjust、remove、clear、confirm。
- 待补交易 UI：已装备物品出售、购物车总价、确认/清空、资金变化预览。
- 基础交易失败提示已覆盖并纳入 `TradeUI` smoke：玩家资金不足、店铺库存不足；待补店铺资金不足、玩家库存不足、不可出售、装备出售需卸下或确认、部分成交策略。
...
- Trade 面板已有店铺/玩家双栏、数量直买直卖和价格预览；待补购物车、拖拽、确认/清空、装备出售。
...
- `TradeUI`：补购物车、批量确认、装备出售和高级失败；店铺/玩家双栏、数量直买直卖、价格预览、资金不足和库存不足提示已有 smoke。
