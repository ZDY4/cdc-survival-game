...
- debug console 第一版已迁移：反引号开关 HUD `DebugConsole`，带输入框、history、suggestions、Tab autocomplete、上下箭头历史浏览、Esc 关闭和 `debug_console` gameplay blocker；`help`、`show fps`、`show overlays`、`observe mode`、`clear`、`restart`、`spawn`、`give item`、`teleport`、`unlock location` 已接入 app 层命令执行，其中运行时命令会复用 Godot `Simulation`、actor registry、库存条目和 overworld unlock，并在变更后重建世界/面板；`UIToggle` smoke 已覆盖成功路径和未知 item / location 拒绝路径。待补命令参数提示、复制/过滤、权限策略、更多参数校验、旧 debug viewer 完整命令集和统一 debug panel。
