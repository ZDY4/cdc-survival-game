...
- action active 时结构性重绘必须进入 TurnActionRunner 的结构变化队列：先完成当前表现 step，再在 runner 稳定边界统一执行地图切换、对象增删、尸体创建或存档刷新。
...
- 世界点击默认阻塞。
- HUD 可刷新。
- Esc 先进入当前 action 的中断流程：完成当前单格 / 当前表现原子段后停在稳定边界，再打开关闭菜单或清除后续 pending action。
...
- action idle 可保存。
- action active 时调用 `prepare_runtime_save_boundary()`，由 TurnActionRunner 驱动当前 action 到稳定边界，再生成存档 snapshot。
