...
- `godot/scripts/app/game_app.gd` 仍约 2668 行，还保留 tooltip / drag facade、hover target reason 补全、overlay 兼容属性、observe / pending 分支、交互结果应用和移动 presentation / refresh 编排等兼容入口。
...
- [ ] 将 `GameApp` 中剩余 hover target reason 补全、兼容 panel 引用等代码继续替换为 `hud_root.apply_runtime_snapshot()`、`hud_root.toggle_*()` 等窄接口。
