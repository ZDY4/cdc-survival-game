...
- 顶层输入分发、逐帧 runtime input process、debug console 输入保护、HUD 面板快捷键和一组全局 UI/debug 快捷键主路径已抽到 `godot/scripts/app/controllers/game_input_router.gd`；`GameRuntimeInputController` 暂保留 direct-call smoke 兼容 fallback。
...
- refresh result 接受和错误消息规范化已抽到 `RuntimeRefreshController.accept_refresh_result()` / `refresh_error_message()`。
...
- [x] refresh result 接受和错误消息规范化已迁入 `RuntimeRefreshController`，`GameApp` 只保留 `push_error` 输出和状态赋值。
