...
- `godot/scripts/app/game_app.gd` 仍约 2141 行，还保留 tooltip / drag snapshot facade、overlay 兼容属性、Space wait / observe 分支、移动/交互等兼容入口。
...
- [ ] 将 Space wait / auto tick wait、移动 / 交互等剩余玩家动作 facade 继续从 `GameApp` 移出。
