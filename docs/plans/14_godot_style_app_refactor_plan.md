...
- `godot/scripts/app/game_app.gd` 仍约 3720 行，还保留大量 UI facade、玩家动作 facade、drag snapshot / hover 计算和 smoke 兼容入口。
...
- 下一步优先抽取 drag snapshot / hover 计算或玩家动作 facade，而不是一次性重命名根脚本。
...
- [ ] 将 drag snapshot / hover 计算、context menu 和 modal blocker 状态继续从 `GameApp` 移出。
