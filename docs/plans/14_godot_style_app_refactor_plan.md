...
- `godot/scripts/app/game_app.gd` 仍约 3096 行，还保留大量 UI facade、玩家动作 facade、context menu 关闭转发和 smoke 兼容入口。
...
- 下一步优先抽取 context menu 关闭转发或玩家动作 facade，而不是一次性重命名根脚本。
...
- [ ] 将 context menu 关闭转发继续从 `GameApp` 移出。
