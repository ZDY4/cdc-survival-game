...
- 新增或修改地图内容后，应该检查哪些同步点
- 哪些内容会自动进入 AI catalog 和 validator
- 哪些 schema 变更不能只改数据
- 出现“AI 看不到新内容”或“proposal 引用新 ID 但 apply/validate 失败”时，优先查哪里
...
- 新增 `world tiles / wall_set / surface_set / prototype`
- 新增 `item_id / character_id / container visual_id`
- 修改地图对象 schema、placement 字段或约束关系
- 地图 AI 没有使用新内容
- proposal 使用了新内容，但 `apply` / `validate` 报 unknown

如果你是在看“后续还有哪些能力没做”，去看 plan 文档，而不是本文档。
