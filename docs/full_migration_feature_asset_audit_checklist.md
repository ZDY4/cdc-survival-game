...
- [~] 背包容量：重量上限第一版已迁移，拾取、容器拿取、商店直买、交易购物车、制作、拆解、装备替换和卸下装备会在 core/economy 统一预检并返回 `inventory_over_capacity`；Inventory / Container / Trade / Crafting UI 已显示负重不足反馈。待补格子、堆叠上限、多 stack、超重惩罚和容器自身容量。
...
- [ ] take/store all。
- [ ] 容器权限：锁、阵营、偷窃、任务状态。
- [~] 容器容量和失败提示：拿取后背包负重不足、容器/背包物品不足、数量非法和权限类失败已有中文反馈。待补容器自身容量、格子限制和更完整权限预览。
- [ ] 空容器表现、清空后地图对象状态。
- [ ] 容器持久化和跨地图保存。
- [ ] 尸体容器与普通容器共用规则。
