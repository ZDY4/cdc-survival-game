...
- [~] 批量制作和即时制作 AP 成本第一版已有；待补制作队列、取消、完成事件、XP/任务推进 polish。
...
- [~] 制作时间 / AP：即时制作 AP 成本第一版已按 `craft_time` 接入玩家命令；待补排队、跨回合、取消、完成事件和时间表现。
...
- [~] 拖拽交易第一版已迁移：shop / inventory / equipment 可拖入购物车或 buy/sell drop zone，drop zone 来源提示、hover 高亮、drag preview 文案和接受/拒绝诊断已纳入 `TradeUI` smoke；待补快捷键和更细业务禁用说明。
...
- [ ] 工作台要求：地图对象、距离、权限、供电。
- [ ] 批量制作和最大可制作数量。
- [~] 制作队列：AP 成本第一版已接入即时制作；待补队列、跨回合时间、取消、完成、离开地图。
...
- [~] `TradeUI`：购物车、批量、价格、失败、无部分成交、装备出售、不可出售和拖拽交易已覆盖；待补快捷键、更细业务禁用说明和统一 drag preview layer polish。
