...
- [ ] 等级曲线、XP 来源、溢出、多级升级。
- [ ] 技能前置、属性要求、互斥、等级上限。
- [~] 主动技能绑定 hotbar、多组/多槽、替换、清除第一版：三组快捷栏、HUD 组按钮、组命名、当前组绑定/使用、独立持久化、HUD/Skills 当前组提示和观察模式入口 / 状态 / 播放 / 速度控制 / 普通热栏隐藏 / 玩家命令拒绝已迁移；待补完整冲突处理。
...
- [~] `src/game_ui/hotbar/**`：快捷栏槽位、多组、HUD 组按钮、组命名、激活、渲染、状态和观察模式入口 / 状态 / 播放 / 速度控制 / 普通热栏隐藏 / 玩家命令拒绝第一版；组级状态 polish 和完整快捷键冲突仍待迁移；落点为 Godot hotbar 和 `use_skill` / `use_item` 命令。
