...
- [~] 背包堆叠、拾取、丢弃、使用、装备/卸下；选中物品操作栏和右键菜单第一版已覆盖数量、使用、装备、丢弃和拖到装备/丢弃按钮。
- [~] 容器打开/关闭、双栏、数量选择、超距关闭、地图切换关闭。
- [~] 交易店铺/玩家双栏、数量买卖、价格预览、购物车、无部分成交。
- [x] 交易中出售已装备物品。
- [~] inventory order、分类、排序、筛选、搜索：inventory order 持久化、默认顺序排序和顺序视图拖拽重排第一版已迁移；分类、名称/重量/价值排序、筛选、搜索已有 UI smoke；待补跨面板拖拽和 polish。
- [~] 背包容量：重量上限第一版已迁移，拾取、容器拿取、商店直买、交易购物车、制作、拆解、装备替换和卸下装备会在 core/economy 统一预检并返回 `inventory_over_capacity`；容器自身容量第一版已迁移，存放前预检重量、总件数和 stack/slot 数限制，超限返回 `container_over_capacity`；Inventory / Container / Trade / Crafting UI 已显示对应反馈。待补背包格子、堆叠上限、多 stack 和超重惩罚。
- [~] 任务物品：不可卖、不可丢、不可使用第一版已迁移；待补不可拆、不可转移和交付条件。
- [ ] 装备槽冲突：双手、盾牌、多饰品、多槽装备。
- [ ] 装备属性派生、装备外观挂点、耐久、损坏。
- [~] 容器权限/容量：锁、钥匙/工具、显式钥匙/工具消耗解锁、world flag、任务状态、NPC owner、关系门槛、允许偷窃、可配置偷窃关系后果、权限预览和容量限制已有第一版；待补逐件工具耐久、NPC 目击、crime system、阵营敌对联动和权限预览 polish。
- [~] take/store all、双向拖拽、快捷转移：全部拿取/全部存放和双向拖拽已有第一版；待补快捷转移和视觉 polish。
...
- [~] 批量制作、UI 制作队列/取消和即时制作 AP 成本第一版已有；待补跨回合生产队列、制作中取消、完成事件、XP/任务推进 polish。
- [~] 拆解输入、产物和 AP 成本第一版已有；待补维修、拆解工具/工作台要求、产物放置和背包满 fallback。
...
- [~] 制作时间 / AP：即时制作 AP 成本第一版已按 `craft_time` 接入玩家命令，UI 制作队列确认时顺序提交即时制作；待补跨回合、制作中取消、完成事件和时间表现。
...
- [~] item count、堆叠、拾取、丢弃。
- [~] UI 列表、详情、基础使用/装备入口；选中物品操作栏和右键菜单已提供使用、装备、丢弃，并支持拖到装备/丢弃按钮。
- [~] inventory order：actor snapshot/save roundtrip、核心增删维护、Inventory 默认顺序排序和顺序视图拖拽重排第一版已迁移；待补筛选/搜索视图提示和 drag polish。
- [~] 分类、排序、筛选、搜索第一版；待补 polish。
- [~] 数量控制：背包选中物品丢弃数量 SpinBox 第一版已迁移；待补拆分、全部丢弃和完整数量弹窗。
- [~] 背包容量：重量上限第一版已迁移，拾取、容器拿取、商店直买、交易购物车、制作、拆解、装备替换和卸下装备会在 core/economy 统一预检并返回 `inventory_over_capacity`；容器自身容量第一版已迁移，存放前预检重量、总件数和 stack/slot 数限制，超限返回 `container_over_capacity`；Inventory / Container / Trade / Crafting UI 已显示对应反馈。待补背包格子、堆叠上限、多 stack 和超重惩罚。
...
- [ ] 双向拖拽。
- [~] take/store all：容器面板“全部拿取 / 全部存放”第一版已迁入，批量命令复用单项容器事务并纳入 `ContainerUI` smoke；待补批量操作视觉 polish。
- [~] 容器权限：锁、钥匙/工具、显式钥匙/工具消耗解锁、world flag、任务状态、NPC owner、关系门槛、允许偷窃、可配置偷窃关系后果和权限预览第一版已迁移；待补逐件工具耐久、NPC 目击、crime system、阵营敌对联动和权限预览 polish。
- [~] 容器容量和失败提示：拿取后背包负重不足、容器/背包物品不足、数量非法、容器自身容量超限、权限预览和权限类失败中文反馈已有第一版。待补格子限制和权限预览 polish。
...
- [ ] 工作台要求：地图对象、距离、权限、供电。
- [~] 批量制作和最大可制作数量第一版。
- [~] 制作队列：UI 队列、单项取消、清空和确认顺序执行第一版已迁移，AP 成本仍由即时制作命令计算；待补跨回合时间、制作中取消、完成、离开地图。
- [ ] 维修：武器、工具、护甲、材料消耗、成功率、耐久。
- [~] 拆解：输入、产物和 AP 成本第一版已有；待补工具/工作台要求、失败原因 UI polish、预览和工具耐久。
- [ ] 产物放置：背包满时进工作台/地面/失败。
- [ ] 制作 XP、技能解锁、任务推进。
...
- [ ] 等级曲线、XP 来源、溢出、多级升级。
- [ ] 技能前置、属性要求、互斥、等级上限。
- [~] 主动技能绑定 hotbar、多组/多槽、替换、清除第一版：三组快捷栏、HUD 组按钮、组命名、当前组绑定/使用、独立持久化、HUD/Skills 当前组提示和观察模式状态 dock 已迁移；待补 observe playback / speed 可操作控制和完整冲突处理。
...
- [~] `CraftingUI`：缺失原因、批量、UI 队列和取消第一版；待补跨回合制作进度和更完整队列 polish。
...
- [~] `src/game_ui/hotbar/**`：快捷栏槽位、多组、HUD 组按钮、组命名、激活、渲染、状态和观察模式状态 dock 第一版；observe playback / speed 可操作控制、组级状态 polish 和完整快捷键冲突仍待迁移；落点为 Godot hotbar 和 `use_skill` / `use_item` 命令。
