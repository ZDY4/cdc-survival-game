...
- [ ] 技能学习确认和失败 reason。
- [~] 多组/多槽 hotbar、拖拽绑定、清空、替换：三组快捷栏、HUD 组按钮、当前组绑定/使用、独立保存和 `Alt+1/2/3` 切组第一版已迁移；待补组命名和完整快捷键冲突矩阵。
...
- [~] `SkillsUI`：技能树、hotbar、多组/多槽、HUD 组按钮、目标预览、cooldown 第一版；待补图形树 pan、组命名和更完整状态 UI。
