...
- [ ] 新增迁移功能时，每一项都需要标注权威层：`data`、`core`、`app`、`world`、`ui` 或 `tools/agent`。
...
Godot 落点：`godot/scripts/**`、`tools/agent/**`。
验收：对应 game/tool smoke 加上 `mainline_migration_guard`。
...
参考：`game_data/src/character.rs`、`appearance.rs`、`ai_preview.rs`。
Godot 落点：`data/characters/*.json`、`data/appearance/*.json`、`godot/scripts/core/actor/**`。
验收：character content validation、runtime bootstrap smoke。
...
- [~] 任务物品片段：不可出售、不可丢弃、不可使用第一版；待补交付条件、不可拆和不可转移。
- [ ] 拆解 / 修理片段：产物、材料、工具、失败提示。
- [ ] 外观片段：preview model、socket、attach target、presentation mode、scale、offset。
参考：`game_data/src/item_edit.rs`、`models.rs`、`appearance.rs`。
...
参考：`game_data/src/skill.rs`、`simulation/skills.rs`。
...
参考：`game_data/src/quest.rs`、`simulation/quest_progression.rs`。
...
参考：`game_data/src/dialogue_runtime.rs`、`dialogue_rules.rs`、`simulation/dialogue.rs`。
...
- [ ] 地图 scene 保存后能被 data layer / topology / world renderer / headless map visual review 同时识别。
...
Godot 落点：`godot/scripts/world/map_object_node.gd`、`world_scene_renderer.gd`。
...
- [~] `Esc` 关闭链路：已覆盖 selection、dialogue、interaction menu、trade modal、container modal、stage panels、settings、pending movement、pending interaction 和无活动 UI 时打开 settings；blocker 诊断已进入 `gameplay_input_blocker_snapshot()` 与 `runtime_control.ui_blocker_snapshot`，现有确认弹窗已进入 `modal_stack_snapshot()`；待补 quantity modal 和 overworld prompt。
...
- [~] console / debug panel / modal / stage panel / context menu 打开时阻止 gameplay 输入。已恢复 debug console、stage/settings、interaction menu、trade panel、container panel、trade equipment sell modal、inventory discard modal 和 blocker name；结构化 blocker snapshot 已暴露 blocked/name/kind/modal_id/panel_id/mouse_blocks_world 并进入 HUD/runtime debug；待补 debug panel、quantity/discard/overworld modal、tooltip/drag 层 blocker 细分。
...
- [~] UI mouse blocker 防止面板点击穿透到世界：stage/settings、interaction menu、trade/container/dialogue panel 和已迁移 modal 会暴露结构化 mouse_blocks_world 诊断并由 `UIToggle` smoke 覆盖；待补 tooltip/drag preview/overworld prompt 等层级。
...
- [~] `UiMenuState` 等价：active stage panel、settings、blocks gameplay input、close stage panels 和 toggle panel 已有运行逻辑；`menu_state_snapshot()` / `runtime_control.menu_state` 暴露 active stage、settings、stage panel 列表、open panels、gameplay blocker 和 close priority，并由 `UIToggle` smoke 覆盖；待补 context menu 与 modal/stage/settings 的完整优先级矩阵。
- [~] `UiModalState` 等价：inventory discard、trade equipment sell 和 skill learn confirm 已进入统一 `modal_stack_snapshot()` / `runtime_control.modal_stack`，并暴露 owner、业务目标、gameplay/mouse blocker；待补 item quantity、container modal 和 overworld prompt。
- [~] `UiContextMenuState` 等价：HUD interaction menu 和库存物品右键菜单已暴露 `context_menu_snapshot()` / `runtime_control.context_menu`，并包含 top/menu 列表、目标、位置、动作、禁用态和 tooltip 摘要；待补装备槽、技能、容器物品、交易行以及完整关闭优先级矩阵。
- [~] `UiHoverTooltipState` 等价：GUI tooltip source、内容和 owner panel 已通过 `hover_tooltip_snapshot()` / `runtime_control.tooltip` 暴露，并覆盖 Character 装备槽与 HUD hotbar；待补屏幕位置、延迟、显隐生命周期、更多面板控件覆盖和 tooltip layer 阻塞/关闭优先级。
- [~] `UiInventoryDragState` 等价：drag source、hover target、payload 和 preview 文本已通过 `drag_state_snapshot()` / `runtime_control.drag` 暴露，覆盖 inventory item、skill hotbar 和 trade item；待补真实 drag preview layer、屏幕位置、threshold、hover 高亮全量覆盖、装备槽可用性细节和 suppress click。
- [~] panel open / close 统一事件和状态行：stage/settings/dialogue/trade/container 可见性变化已进入 `menu_state_snapshot().recent_events/latest_event` 与 `runtime_control.menu_state`，HUD runtime/debug 行显示 `Panel opened/closed:<panel>`，并由 `UIToggle` smoke 覆盖；待补 toast/feed 表现、modal/context menu 同步事件和更完整关闭原因。
- [~] 同一时刻 modal、context menu、stage panel 的优先级和关闭规则稳定：modal stack、menu state 和 blocker snapshot 已覆盖现有确认弹窗、stage/settings、interaction menu 的第一版关闭/阻塞诊断；待补完整 context menu、quantity modal、overworld prompt 与 drag/tooltip 层级矩阵。
...
- [~] 敌对、同层、距离校验；攻击执行和预览已暴露 same_level、range_ok、min_range_ok、line_of_sight 与 spatial_failure 诊断字段。
...
- [ ] buff / debuff 对伤害的影响。
- [~] 装备 / on-hit 特效第一版：`equip_effect_ids` 已进入角色装备快照，`ammo_capacity` / `reload_speed` 已影响玩家已装备武器换弹容量和 AP 成本；武器 on-hit effect 已能把有持续时间或 modifier 的状态写入目标 `active_effects`，支持 refresh / extend / intensity 基础堆叠。待补 DoT tick、stun 行动限制、即时公式类 effect、武器耐久和弹药特效。
...
- [~] 背包列表、数量移动、丢弃第一版；选中物品操作栏和右键上下文菜单已提供使用、装备、丢弃，背包物品可拖到装备/丢弃按钮，并纳入 `InventoryUI` smoke。
- [~] inventory order 持久化第一版：actor snapshot / save roundtrip、核心物品增删顺序维护、Inventory 默认“顺序”排序和顺序视图拖拽重排已纳入 `InventoryUI` / `Save` smoke；待补筛选/搜索视图提示和拖拽 polish。
- [~] 排序、筛选、搜索、分类第一版；顺序、名称、重量、价值排序已覆盖，待补更完整 UI polish。
- [x] 选中物品详情，已纳入 `ContainerUI` smoke。
- [~] 上下文入口：选中物品操作栏和右键菜单已覆盖使用、装备、丢弃；待补完整右键上下文菜单：拆分、检查、加入热栏、出售、存入容器。
- [~] 数量控制：背包丢弃数量 SpinBox 第一版已迁移；待补完整数量弹窗：增减、最大、确认、取消、非法提示。
- [~] 物品使用：`use_item` 命令、消耗品资源恢复、AP 消耗、消耗物品、失败 reason、任务/关键物品不可使用和 Inventory 使用按钮第一版已纳入 `InventoryUI` / `Save` smoke；待补 buff/debuff、持续效果、任务交付限制。
- [~] 拖拽排序、拖到装备按钮、拖到丢弃按钮第一版已迁移；排序走 `reorder_inventory` core 命令；待补拖到实际装备槽、拖到容器、拖到交易、独立丢弃区。
- [~] 容量、重量、格子限制：背包重量上限第一版已迁入 `core/economy`，拾取、容器拿取、商店直买、交易购物车、制作、拆解、装备替换和卸下装备会统一预检并返回 `inventory_over_capacity`；容器自身容量第一版已迁入存放规则，支持重量、总件数和 stack/slot 数限制，超限返回 `container_over_capacity` 且不修改库存；待补背包格子限制、多 stack 堆叠上限和超重惩罚。
...
- [~] 权限：锁定、钥匙/工具、world flag、任务限制、NPC owner、关系门槛、允许偷窃和可配置偷窃关系后果第一版已迁移；待补 NPC 目击、crime system 和阵营敌对联动。
...
- [x] 装备出售：装备栏物品可作为 `equipment:<slot_id>` 来源出售，出售前确认，确认后卸下并成交，已纳入 `TradeUI` smoke。
- [~] 买价 / 卖价倍率已用于价格预览；待补关系和技能影响价格。
- [~] 购物车总价已显示应付/应收/净付/净收，确认前库存/资金预校验和无部分成交已纳入 `TradeUI` smoke；待补资金变化明细。
- [x] 交易资金/库存失败提示已纳入 `TradeUI` smoke：玩家资金不足、店铺资金不足、店铺库存不足、玩家库存不足。
- [x] 不可出售和装备出售确认：直卖、装备出售、购物车和 UI 禁用态第一版已纳入 `TradeUI` smoke。
- [x] 拖拽：shop -> buy zone、inventory/equipment -> sell zone、cart 重排、drop zone 来源提示、hover 高亮和最近一次拖拽接受/拒绝预览第一版已纳入 `TradeUI` smoke。
- [x] 交易快捷键：`Q` 入购物车、`Delete` 清空、`Enter` 直交易、`Shift+Enter` 确认购物车已纳入 `TradeUI` smoke。
...
- [~] observe mode hotbar：模式切换、播放、速度、自动状态和观察楼层展示第一版已迁移，Observe / Player、Play、Speed、Auto 按钮可操作，速度会影响自动推进间隔，Space 可切换播放；普通 hotbar 会在 observe mode 下隐藏，player command 统一拒绝。待补完整冲突策略。
...
参考：`game_bevy/src/asset_paths.rs`、`world_render/tile_assets.rs`。
...
- [x] 字体：旧 `assets/fonts/NotoSansCJKsc-Regular.otf` 已迁入 `godot/assets/fonts/NotoSansCJKsc-Regular.otf`；待守护 Godot `.import`、UI 字体 fallback 和中文缺字。
...
## 17. Agent workflow 和脚本

- [~] `tools/agent/godot-content.ps1`、`review-godot-map-visual.ps1`、`test-godot-game.ps1`。
...
- [~] debug console：反引号开关、输入框、history、schema-driven suggestions、命令参数提示、Tab autocomplete、上下箭头历史浏览、Esc 关闭和 gameplay blocker 第一版已迁移，并由 `UIToggle` smoke 覆盖；待补完整视觉和复制/过滤。
- [~] console commands：`help`、`show fps`、`show overlays`、`observe mode`、`clear`、`restart`、`spawn`、`give item`、`teleport`、`unlock location` 第一版已迁移；命令 schema 已集中在 app runner 并驱动 help、suggestions 和参数提示，运行时修改命令已标记 `debug_runtime_mutation` 权限并可通过 `cdc/debug_console/allow_runtime_mutation` 统一禁用，数量/坐标参数改为严格整数校验，`UIToggle` smoke 覆盖成功、权限拒绝、非法参数和常见拒绝路径；待补旧 debug viewer 的完整命令集。
...
- [~] selection debug：hovered grid、actor、object、blocker name 和 prompt summary 已进入 `runtime_control.selection_debug`，结构化 UI blocker 已进入 `runtime_control.ui_blocker_snapshot` 与 F3 debug panel runtime 行，HUD runtime 行显示 `Sel ...` 摘要，并由 `PlayerInteraction` / `UIToggle` smoke 覆盖；待补统一 debug panel 页面和更完整 prompt 细节。
...
- [~] `SkillsUI`：技能树、hotbar、多组/多槽、HUD 组按钮、组命名、目标预览、cooldown 第一版；待补图形树 pan 和更完整状态 UI。
...
9. Console、info panels、profiling 和开发观察工具。
