...
func attack_presentation(events: Array, world_root: Node, world_result: Dictionary) -> Dictionary:
...
func start_attack_feedback(host: Node, world_root: Node, attack: Dictionary) -> void:
	var target_node: Node3D = attack.get("target_node", null)
	if target_node == null:
		_record_latest(attack_public_snapshot(attack, false, "target_node_missing"))
		return
	...
	var snapshot_data := attack_public_snapshot(attack, true, "")
...
func attack_public_snapshot(attack: Dictionary, active: bool, reason: String) -> Dictionary:
	return {
		"active": active,
		"kind": "attack",
		"reason": reason,
