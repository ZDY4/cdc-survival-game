...
func submit_attack(simulation: RefCounted, actor: RefCounted, command: Dictionary) -> Dictionary:
	...
	if attack_distance > attack_range:
		...
		return simulation._approach_then_execute_interaction(actor, source_target, source_option_id, prompt, _dictionary_or_empty(command.get("topology", {})))
	if attack_distance < min_range:
		return simulation.perform_attack(actor.actor_id, target_actor_id, _dictionary_or_empty(command.get("topology", {})), _attack_perform_options(attack_range, min_range, profile, attack_options))
