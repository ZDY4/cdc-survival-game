...
旧 `open-editor.ps1`、`review-map-visual.ps1`、`test-bevy-game.ps1` 和 `smoke_bevy_editors.ps1` 已归档到 `legacy/bevy/agent/`，只在需要 Rust/Bevy 行为差异对照时使用。

## Tools

### Legacy Bevy Tools

旧 Bevy 对照脚本已移出默认工具目录：

- `legacy/bevy/agent/open-editor.ps1`
- `legacy/bevy/agent/review-map-visual.ps1`
- `legacy/bevy/agent/test-bevy-game.ps1`
- `legacy/bevy/agent/smoke_bevy_editors.ps1`

只在需要旧 Rust/Bevy 行为差异分析时使用这些脚本；迁移开发默认继续使用下面的 Godot 工具。
