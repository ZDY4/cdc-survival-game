...
- `CDC Content Browser` dock 会在 Godot editor 内浏览 `item` / `recipe` / `character` / `map`，并显示记录级校验状态、详情和可编辑字段清单。
- 内容保存边界已收口到 `godot/scripts/data/content_edit_service.gd`；当前 dock 仍先保持只读，后续表单 UI 通过该服务写回 JSON。
...
- `validate` 对 `item` / `recipe` / `character` / `map` 执行记录级诊断，输出 `relative_path`、`status` 和字段级 issue；`validate changed` 会批量检查这些已迁移编辑内容。
