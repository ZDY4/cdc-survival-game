...
- 内容定位、摘要、引用、校验、格式化、diff 摘要和资产 manifest：`godot-content.ps1`
...
- 游戏运行时 profiling：`test-godot-profile.ps1`
...
- 需要 Godot 可执行文件的 agent 脚本通过 `tools/agent/godot-env.ps1` 解析命令：显式 `-Godot` 参数优先，其次读取 `GODOT` 环境变量，然后查找 PATH 中的 `godot` / `godot.exe` / `godot.cmd`，最后 fallback 到 `D:\godot\godot.cmd`。
...
- `Import` 通过解析出的 Godot 命令调用 `--headless --editor --import --quit --path godot`。
- `CheckOnly` 会遍历 `godot/**/*.gd`，逐个调用解析出的 Godot 命令执行 `--headless --path godot --check-only --script res://...`。
...
- 若没有最近 session，则启动解析出的 Godot 命令执行 `--editor --path godot`。
...
- 通过 Godot headless content CLI 执行内容定位、摘要、引用、校验、格式化、diff 摘要和资产 manifest，作为 `content_tools` 的迁移替代入口。
...
- 通过解析出的 Godot 命令调用 `--headless --path godot --script res://scripts/tools/content_cli.gd -- ...`。
...
```powershell
& $env:GODOT --headless --path godot --script res://scripts/tools/world_tile_resource_migration.gd -- --dry-run
& $env:GODOT --headless --path godot --script res://scripts/tools/world_tile_resource_migration.gd
...
- 通过解析出的 Godot 命令调用 `--headless --path godot --script <smoke>`。
...
- 通过解析出的 Godot 命令调用 `--headless --path godot --script <smoke>`；`HeadlessNewGame` / `HeadlessWorld` 通过 `godot/scripts/app/headless_runner.gd` 覆盖 headless 启动入口。
- 默认 runtime scene 入口为 `godot/scenes/game/game_root.tscn`。
...
### `test-godot-profile.ps1`

用途：

- 运行 Godot runtime profiling 场景，重复真实运行时操作并记录帧耗时、FPS、节点/渲染计数、路径搜索指标和关键 GDScript 函数耗时。

何时使用：

- 怀疑运行时帧率随操作次数下降、输入/移动/回合 runner 变慢或需要对比性能趋势时。
- smoke 已通过但仍需要量化热点函数和退化趋势时。
...
- 默认通过解析出的 Godot 命令调用 `--path godot --script res://scripts/tools/runtime_profile_probe.gd --disable-vsync --max-fps 0`。
- 默认以可见窗口运行，以便真实输入事件、相机、physics picking 和帧循环更接近交互 runtime；需要 CI/headless 时可加 `-Headless`。
- 当前覆盖 `MovementClickRepeat`：在启动地图上重复点击可达地面，等待玩家控制角色到达目标点，并记录每轮指标。
- 输出 console log、profile JSON 和 wrapper result JSON 到 `.local/agent-smoke/godot_profile/<timestamp>/`。
