...
- 内容保存边界已收口到 `godot/scripts/data/content_edit_service.gd`；当前表单保存覆盖 `item` / `recipe` / `character` / `map` / `quest` / `skill` / `skill_tree` 的安全元数据字段，`dialogue` / `settlement` / `overworld` 先保持只读。
